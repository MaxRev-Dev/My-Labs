...
int main() {
	...
	for (int i = 0; i < cols; i++) {
		...
		for (int j = 0; j < rows; j++) {
			cout << "¬вед≥ть A["<<i+1<<"]["<<j+1<<"]: ";
			cin >> m[i][j];
		}
	}
	//вив≥д
	cout << endl<<"¬аша матриц€"<<endl;
	for (int i = 0; i < cols; i++) {
		for (int j = 0; j < rows; j++) {
			cout << m[i][j] << " ";
		}cout << endl;
	}
	//пошук максимальних елемент≥в
	vector<int> maxline;
	for (int i = 0; i < cols; i++) {
	...
	cout <<"—ума дор≥внюЇ " << sum<<endl;
	//запис суми у д≥агональ
	for (int i = 0; i < cols; i++) {
		m[i][i] = sum;
	}
	//вив≥д
	cout <<endl<<"ѕеретворена матриц€"<<endl;
	for (int i = 0; i < cols; i++) {
		for (int j = 0; j < rows; j++) {
			cout << m[i][j] << " ";
		}cout << endl;
